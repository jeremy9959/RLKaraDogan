...
def run_episode_for_plotting(agent, supply_chain, k, theta, time_units_per_episode):
    ...
    p5.line(episode_data["time"], episode_data["inventory_size"], legend_label="Inventory Size", line_width=2)
    ...
    hist_units_ordered, edges_units_ordered = np.histogram(episode_data["units_ordered"], bins=50)
    ...
    hist_expired_units, edges_expired_units = np.histogram(episode_data["expired_units"], bins=50)
    ...
    hist_unmet_demand, edges_unmet_demand = np.histogram(episode_data["unmet_demand"], bins=50)
    ...
    p9 = figure(title="Inventory Size Histogram", x_axis_label="Inventory Size", y_axis_label="Frequency")
    hist_inventory_size, edges_inventory_size = np.histogram(episode_data["inventory_size"], bins=50)
    p9.quad(top=hist_inventory_size, bottom=0, left=edges_inventory_size[:-1], right=edges_inventory_size[1:], fill_color="navy", line_color="white", alpha=0.7)
...
def plot_total_rewards(rewards_per_episode):
    ...
    """
    p2 = figure(title="Avg Reward vs Episodes", x_axis_label="Episodes", y_axis_label="Total Rewards")
    p2.line(list(range(1, len(rewards_per_episode) + 1)), rewards_per_episode, legend_label="Total Rewards", line_width=2)
